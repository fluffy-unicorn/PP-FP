...
parserGen gr (nt:rule) (nt0,ts, allTokens@((k,(cat,str)):remTokens), recCheck)
    ...
    | otherwise             = case nt of
        ...
        Terminal str'   | str==str'     -> parserGen gr rule (nt0, ts++[PLeaf (cat,str)], remTokens, [])
...
parse gr s tokens | null correctParses = maximum $ map fst parses
                  | null totalParses   = PError final' [] None
                                                ("tokenlist not fully parsed. Still left: " ++ (show $ map snd rest'))
                                                0
                  | otherwise          = final
          where
            parses = [ (t,rem) | r <- gr s
            ...
            totalParses   = filter (null . snd) correctParses

            (final ,rest )  = head totalParses
